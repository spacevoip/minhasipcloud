...
if (!process.env.POST_PASSWORD) {
  logger.error('❌ POST_PASSWORD é obrigatória no arquivo .env');
  process.exit(1);
}

// Pool de conexão PostgreSQL com tratamento de erro
const pool = new Pool({
  host: process.env.POST_HOST || '38.51.135.181',
  port: process.env.POST_PORT || 6543,
  database: process.env.POST_DATABASE || 'postgres',
  user: process.env.POST_USER || 'postgres.supabase',
  password: process.env.POST_PASSWORD,
  max: process.env.POST_MAX || 20,
...
// ================================
// ROUTES - TRANSFERÊNCIA DE CHAMADAS
// ================================

// Importar e registrar rotas de transferência
const callTransferRoutes = require('./backend/src/routes/callTransfer');
